...
const ExamScreen = ({ navigation, route }) => {
  ...
  const loadExamQuestions = async () => {
    try {
      // Mock exam questions
      const response = await examAPI.getQuestionsForSkill(skill || 'JavaScript');

      if (response.data.success) {
        console.log('✅ EXAM DATA RECEIVED FROM AI:', response.data.data); // DEBUG LOG
  ...
  const submitExam = async () => {
    ...
    try {
      setLoading(true);
      // We fire and forget the save, or await it. Await is safer.
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={styles.loadingContainer}>
          <Text style={[styles.loadingText, { color: colors.text }]}>
            {examStarted ? 'Submitting Exam...' : 'Preparing your exam...'}
          </Text>
        </View>
      </SafeAreaView>
    );
