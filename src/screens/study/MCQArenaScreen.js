  ...
  useEffect(() => {
    if (quizFinished) {
      ...
      if (score >= passingScore) {
        // Attempt to extract Day Number from topic string or use logic. 
        // Since we don't have dayNumber easily here, we might need to pass it via route params.
        // Assume route.params.dayNumber exists or extract from Topic.
        // Wait, TopicTimelineScreen passes { topic } object which has .day.

        // Let's check how navigation happens. TopicDetailScreen -> navigate('MCQArena', { topic: topic.title })
        // We lost the day number! We need to pass it through.

        const { dayNumber } = route.params || {};
        if (dayNumber) {
          import('../../services/api').then(({ studyAPI }) => {
            studyAPI.completeDay(dayNumber).catch(e => console.error("Complete Day Failed", e));
          });
        }
      }
    }
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background, padding: 20, justifyContent: 'center' }]}>
        <Card style={{ alignItems: 'center', padding: 30 }}>
          ...
          <View style={{ width: '100%', gap: 10 }}>
            ...
            <Button title="Back to Study Plan" onPress={() => navigation.navigate('Study')} />
          </View>
        </Card>
      </SafeAreaView>
    );
